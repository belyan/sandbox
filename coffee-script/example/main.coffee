...
class World
            ...
            if @controllable instanceof ParticleSystem
                if e.shiftKey
                    @controllable.scatter =
                        Math.max 0, @controllable.scatter - e.wheelDelta / 100
                else if e.altKey
                    @controllable.particleSize =
                        Math.max 0, @controllable.particleSize - e.wheelDelta / 100
                else
                    @controllable.particleLife =
                        Math.max 1, @controllable.particleLife - e.wheelDelta / 10
        ), no
    ...
    update: ->
        object.update index for object, index in @objects when object
        return
...
class ParticleSystem extends Obj
    constructor: (config) ->
        super config
        @particles = []
    ...
    update: ->
        ...
        particle.update index for particle, index in @particles when particle
        return
...
class Particle extends Obj
    ...
    update: (index) ->
        ...
        if (@notVisible 100) or (@life < 0) then @system.removeParticle index
